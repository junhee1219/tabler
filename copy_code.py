...
def build_tree_lines(root: Path, prefix: str = "") -> list[str]:
    ...
    entries = sorted(
        [p for p in root.iterdir()
         if p.name != "node_modules" and not p.name.startswith(".") and not p.name.startswith("libs") and not p.name.startswith("dist")
...
def main():
    ...
    if not ts_files:
        ...
        for file_path in ts_files:
            ...
            if str(rel) == "package-lock.json" or str(rel).startswith(".") or str(rel).startswith("node_modules") or str(rel).startswith("libs") or str(rel).startswith("dist"):
                continue
